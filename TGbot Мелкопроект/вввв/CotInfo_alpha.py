...
def parse_config(what_parse):
    token = str()
    with open(fr'{os.getcwd()}\bot_config.txt', 'r') as fol:
        file = fol.read().split('\n')
        for i in file:
            if what_parse in i:
                token = i.split(f'{what_parse} = ')[1]
    return token
...
def file_check_copy_sub(adres, id): # Проверяет есть ли котельная в файле
    ad = list()

    try:
        with open(f'{os.getcwd()}\Subscribers\Subscribe_{id}.txt', 'r', encoding='utf-8') as fol:
            file = fol.read().replace('sub = ', '')
            ad =  [line.split(', ')[0] for line in file.split('\n') if line != '' or 'sub' in line]  
            #(adres, ad, id)
            if adres in ad:
                return True
            else:
...
def file_check_copy_fav(adres, id): # Проверяет есть ли котельная в файле
    ad = list()

    try:
        with open(f'{os.getcwd()}\Subscribers\Subscribe_{id}.txt', 'r', encoding='utf-8') as fol:
            file = fol.read().replace('fav = ', '')
            ad =  [line.split(', ')[0] for line in file.split('\n') if line != '' or 'fav' in line]  
            #(adres, ad, id)
            if adres in ad:
                return True
            else:
...
def subscribe_del(adres, id):  # Удаление котельной из подписок # adres - adres # id - message.from_user.id
    ...
    try:
        ...
        with open(f'{os.getcwd()}\Subscribers\Subscribe_{id}.txt', 'r', encoding='utf-8') as fol:  
            file = fol.read().replace('sub = ', '')
            # #(f'{os.getcwd()}\Subscribers\Subscribe_{id}.txt') 
            # #([line for line in fol.read().split('\n') if line != adres])
...
def send_message_interval_5(adres, id): # Добавление интервала между сообщениями в 5 минут
    subscribe = list()
    try:
        #('try')
        with open(f'{os.getcwd()}\Subscribers\Subscribe_{id}.txt', 'r', encoding='utf-8') as fol:  
            ...
            #(adres)
        with open(f'{os.getcwd()}\Subscribers\Subscribe_{id}.txt', 'w', encoding='utf-8') as new_fol: # Добавление котельной в файл с подписками
            # subscribe = [line.replace(', 10', ', 5') if (line.split(',')[0] == adres) and (', 5' not in str(full_adres)) else line for line in a ] # Добавление цифры 5 - интервал в минутах по присыланию сообщения
            # subscribe = list()
            for line in a:
                if (line.split(', ')[0] == adres) and (', 5' not in line): # Проверка на наличие этого промежутка времени
                    if ', 10' in line:
                        subscribe.append(line.replace(', 10', ', 5'))
                    else:
                        subscribe.append(line+', 5')
                else:
                    subscribe.append(line)        
            
            #([#(line, adres) for line in a ])
            subscribe.sort()
            # #(a.sort())
            #(subscribe)
            for i in subscribe:
...
def send_message_interval_10(adres, id): # Добавление интервала между сообщениями в 10 минут
    subscribe = list()
    try:
        #('try')
        with open(f'{os.getcwd()}\Subscribers\Subscribe_{id}.txt', 'r', encoding='utf-8') as fol:  
        ...
        with open(f'{os.getcwd()}\Subscribers\Subscribe_{id}.txt', 'w', encoding='utf-8') as new_fol: # Добавление котельной в файл с подписками
            ...
            subscribe = list()
            for line in a:
                if (line.split(', ')[0] == adres) and (', 10' not in line): # Проверка на наличие этого промежутка времени
                    if ', 5' in line:
                        subscribe.append(line.replace(', 5', ', 10'))
                    else:
                        subscribe.append(line+', 10')
                else:
                    subscribe.append(line) 
...
# #(add_newtime('Ченцова 71_63а(3)', 5288656086))
#
...
# #(file_new_time(5288656086, 'Врубовая'))
